...
int getPixelDepth(){
	return 4;//TODO Change to make dynamic
}
int getPitch(){
	return width*getPixelDepth();
}
void plot_pixel(unsigned int pixel,unsigned int x,unsigned int y){
	if(sizeof(unsigned int) < 4){
		return 0;
	}
	*(uint32_t*)(0xb8000 + y*getPitch()+x*4) = pixel;
}
